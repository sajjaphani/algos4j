...
/**
 * An undirected weighted graph, which uses adjacency matrix representation.
 * 
 * @author psajja
 *
 */
public class WeightedGraphAdjMatrix extends AbstractWeightedGraph {
	...
	public void addEdge(int fromVertex, int toVertex, int weight) {
		...
		if (adjMatrix[fromVertex][toVertex] == null) {
			...
			adjMatrix[toVertex][fromVertex] = new Edge(toVertex, fromVertex, weight);
		}
	}
	...
	public void removeEdge(int fromVertex, int toVertex) {
		...
		if (adjMatrix[fromVertex][toVertex] != null) {
			...
			adjMatrix[toVertex][fromVertex] = null;
		}
	}
