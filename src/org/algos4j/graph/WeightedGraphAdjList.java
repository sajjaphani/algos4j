...
/**
 * An undirected weighted graph, which uses adjacency list representation.
 * 
 * @author psajja
 *
 */
public class WeightedGraphAdjList extends AbstractWeightedGraph {
	...
	public void addEdge(int fromVertex, int toVertex, int weight) {
		...
		if (!hasEdge(fromVertex, toVertex)) {
			...
			if(toVertex != fromVertex)
				adjList[toVertex].add(new Edge(toVertex, fromVertex, weight));
		}
	}
	...
	public void removeEdge(int fromVertex, int toVertex) {
		validate(fromVertex);
		validate(toVertex);
		// Remove edge (fromVertex -> toVertex)
		int index = getIndex(adjList[fromVertex], fromVertex, toVertex);
		if(index != -1) {
			decrementEdges();
			adjList[fromVertex].remove(index);
			// Remove edge (toVertex -> fromVertex)
			int index1 = getIndex(adjList[toVertex], toVertex, fromVertex);
			adjList[toVertex].remove(index1);
		}
	}
